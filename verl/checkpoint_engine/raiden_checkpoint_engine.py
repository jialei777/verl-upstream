...
def _compute_tensor_checksum(items) -> dict:
    ...
    for item in items:
        ...
        raw_bytes = p_cpu.flatten().view(torch.uint8).numpy().tobytes()
        t_hash = hashlib.sha256(raw_bytes).hexdigest()
        hasher.update(raw_bytes)
        t_l1 = float(p_cpu.float().abs().sum().item())
        t_l2_sq = float(p_cpu.float().pow(2).sum().item())
        if name is not None:
            per_tensor[name] = {
                "sha256": t_hash,
                "l1": t_l1,
...
class RaidenCheckpointEngine(CheckpointEngine):
    ...
    def __init__(self, bucket_size: int = 0, is_master: bool = False, **kwargs) -> None:
        ...
        self._saved_local_port = 0
        self._saved_listener_port = 0
    ...
    async def send_weights(
        ...
        **kwargs,
        ...
        logger.info(f"@@@ RaidenCheckpointEngine: [Step {step_key}] Start send_weights on rank {self.rank}...")

        if hasattr(torch, "tpu") and hasattr(torch.tpu, "synchronize"):
            torch.tpu.synchronize()

        # Every rank in actor_wg MUST consume the generator so TorchTitan/FSDP full_tensor() collectives finish.
        weight_list = list(weights)

        sampler_meta_map = {}
        if self.registry is not None:
            try:
                sampler_meta_map = ray.get(self.registry.get_sampler_meta.remote()) or {}
            except Exception as e:
                logger.warning(f"Trainer Rank {self.rank}: Could not query sampler_meta from TPUWeightRegistry: {e}")

        if sampler_meta_map and self.rank not in sampler_meta_map:
            logger.info(
                f"Trainer Rank {self.rank}: No corresponding Sampler rank in {sorted(sampler_meta_map.keys())}; "
                "consumed weights generator and returning."
            )
            return

        tp_size = max(1, len(sampler_meta_map))
        tp_rank = self.rank if self.rank < tp_size else 0
        rank_meta = sampler_meta_map.get(tp_rank, {})
        flipped_keys = set(rank_meta.get("flipped_keys", []))
        target_shapes = rank_meta.get("shapes", {})
        ...
        # Pack un-fused QKV and MLP projections to match vLLM's MergedColumnParallelLinear modules
        weight_dict = dict(filtered_weight_list)
        packed_weights = {}
        processed_keys = set()
        for k, v in filtered_weight_list:
            if k in processed_keys:
                continue
            if ".self_attn.q_proj.weight" in k:
                k_key = k.replace(".self_attn.q_proj.weight", ".self_attn.k_proj.weight")
                v_key = k.replace(".self_attn.q_proj.weight", ".self_attn.v_proj.weight")
                qkv_key = k.replace(".self_attn.q_proj.weight", ".self_attn.qkv_proj.weight")
                if k_key in weight_dict and v_key in weight_dict:
                    q = weight_dict[k]
                    k_t = weight_dict[k_key]
                    v_t = weight_dict[v_key]
                    q_data = q.to_local().data if hasattr(q, "to_local") else (q.data if hasattr(q, "data") else q)
                    k_data = k_t.to_local().data if hasattr(k_t, "to_local") else (k_t.data if hasattr(k_t, "data") else k_t)
                    v_data = v_t.to_local().data if hasattr(v_t, "to_local") else (v_t.data if hasattr(v_t, "data") else v_t)
                    if tp_size > 1:
                        q_data = q_data.chunk(tp_size, dim=0)[tp_rank]
                        k_data = k_data.chunk(tp_size, dim=0)[tp_rank]
                        v_data = v_data.chunk(tp_size, dim=0)[tp_rank]
                    qkv = torch.cat([q_data, k_data, v_data], dim=0)
                    packed_weights[qkv_key] = qkv
                    processed_keys.update([k, k_key, v_key])
                    continue
            elif ".mlp.gate_proj.weight" in k:
                up_key = k.replace(".mlp.gate_proj.weight", ".mlp.up_proj.weight")
                gate_up_key = k.replace(".mlp.gate_proj.weight", ".mlp.gate_up_proj.weight")
                if up_key in weight_dict:
                    gate = weight_dict[k]
                    up = weight_dict[up_key]
                    gate_data = gate.to_local().data if hasattr(gate, "to_local") else (gate.data if hasattr(gate, "data") else gate)
                    up_data = up.to_local().data if hasattr(up, "to_local") else (up.data if hasattr(up, "data") else up)
                    if tp_size > 1:
                        gate_data = gate_data.chunk(tp_size, dim=0)[tp_rank]
                        up_data = up_data.chunk(tp_size, dim=0)[tp_rank]
                    gate_up = torch.cat([gate_data, up_data], dim=0)
                    packed_weights[gate_up_key] = gate_up
                    processed_keys.update([k, up_key])
                    continue
        ...
        for name, p in sorted_weights:
            ...
            # Match vLLM 0.29 VllmUnquantizedLinearMethod 2D weight transposition (_tpu_weight_flipped=True)
            is_flipped = (
                name in flipped_keys
                or any(name == fk or name.endswith("." + fk) for fk in flipped_keys)
            )
            if is_flipped and t.dim() == 2:
                t = t.transpose(0, 1)

            # Match TP shard shape for RowParallelLinear / VocabParallelEmbedding when tp_size > 1
            target_shape = target_shapes.get(name)
            if target_shape is not None and list(t.shape) != list(target_shape):
                for dim in range(t.dim()):
                    if t.shape[dim] != target_shape[dim] and t.shape[dim] == target_shape[dim] * tp_size:
                        t = t.chunk(tp_size, dim=dim)[tp_rank]
                        break
        ...
        if getattr(self, "_trainer_raiden_ws", None) is not None and getattr(self, "_trainer_tensors", None):
            for dst_t, (_, src_t) in zip(self._trainer_tensors, valid_weights):
                if dst_t.data_ptr() != src_t.data_ptr():
                    dst_t.copy_(src_t)
        else:
            ...
            logger.info(
                ...
                f"(sample tensor: name={valid_weights[0][0]}, shape={list(valid_weights[0][1].shape)}, "
                f"device={valid_weights[0][1].device}, dtype={valid_weights[0][1].dtype}, "
                f"total numel={sum(t.numel() for t in tensors)})"
            )

            self._trainer_raiden_ws = _create_torch_weight_synchronizer(
                device_tensors,
                local_port=getattr(self, "_saved_local_port", 0),
                listener_port=getattr(self, "_saved_listener_port", 0),
                parallelism=8,
                bind_ip=bind_ip,
            )
            self._saved_local_port = self._trainer_raiden_ws.local_port
            self._saved_listener_port = self._trainer_raiden_ws.listener_port
            ...
            data_name = f"weights_{self.rank}"
            if controller_addr and ":" in controller_addr:
                ...
                try:
                    ...
                    unit_id = raiden_controller.RaidenId("trainer", str(self.rank), data_name)
                    ...
                    logger.error(
                        f"Trainer Rank {self.rank} failed to register with RaidenController ({controller_addr}): {reg_err}"
                    )
                    raise
            else:
                raise RuntimeError(
        ...
        # Ensure device operations are completely finished and pending graphs evaluated before D2H DMA
        try:
        ...
        try:
            hb = self._trainer_raiden_ws.get_host_buffer(0, 0)
            hb_l1 = float(hb.float().abs().sum().item())
            m = self._trainer_raiden_ws.get_metrics()
            print(
                f"@@@ [RAIDEN DIAG | Trainer] Step {step_key} Rank {self.rank}: d2h_wall={t_d2h:.4f}s, "
                f"host_buf(0,0) L1={hb_l1:.6f}, metrics: d2h_count={m.get('d2h_call_count')}, "
                f"last_d2h_bytes={m.get('last_d2h_bytes')}, total_d2h_bytes={m.get('total_d2h_bytes')}, "
                f"total_h2h_bytes={m.get('total_h2h_bytes')}",
                flush=True,
            )
        except Exception as diag_e:
            print(f"@@@ [RAIDEN DIAG | Trainer] Could not inspect host buffer/metrics: {diag_e}", flush=True)

        # Compute deterministic checksum & norms across rank 0 sent tensors in background
        if compute_checksum and self.registry is not None and self.rank == 0:
            try:
                cpu_items = [(name, p.detach().cpu().contiguous()) for name, p in valid_weights]
                loop = asyncio.get_event_loop()
                loop.run_in_executor(None, self._record_checksum_bg, step_key, cpu_items)
            except Exception as sched_err:
                logger.warning(f"Failed to schedule checksum recording: {sched_err}")

    def _record_checksum_bg(self, step_key: int, cpu_items) -> None:
        """Background worker thread to calculate SHA256 checksum and post to registry."""
        try:
            trainer_checksum = _compute_tensor_checksum(cpu_items)
...
async def update_raiden_weights(manager, global_steps: Optional[int] = None) -> dict:
    ...
    # 2. Trigger Trainer ranks to register their tensors with central RaidenController and run D2H DMA
    ...
    sampler_meta_map = await registry.get_sampler_meta.remote()
    active_ranks = sorted(sampler_meta_map.keys()) if sampler_meta_map else [0]

    from tpu_sync.api.common import RaidenId
    from tpu_sync.rpc.raiden_controller import RaidenMemoryType

    src_units = [
        RaidenId(job_name="trainer", job_replica_id=str(r), data_name=f"weights_{r}")
        for r in active_ranks
    ]
    dst_units = [
        RaidenId(job_name="sampler", job_replica_id=str(r), data_name=f"weights_{r}")
        for r in active_ranks
    ]
    ...
    if hasattr(manager, "raiden_controller") and manager.raiden_controller is not None:
        ...
        transfer_futures = [
            manager.raiden_controller.start_transfer(
                ...
                parallelism=8,
                req_id=f"verl_step_{global_steps or 0}_r{r}",
            )
            for r, s_u, d_u in zip(active_ranks, src_units, dst_units)
        ]
        await asyncio.gather(*[f.wait() for f in transfer_futures])
    ...
    print(telemetry_msg, flush=True)
    ...
    try:
        ...
        if not trainer_checksum or not sampler_checksums:
            logger.warning(
                ...
                f"trainer_checksum={trainer_checksum is not None}, sampler_checksums={sampler_checksums}"
            )
            return
        ...
        # Compare Trainer rank 0 checksum against Sampler rank 0 checksum
        s_meta = flat_sampler_checksums[0] if flat_sampler_checksums else None
        if isinstance(s_meta, dict):
            t_sha = trainer_checksum.get("sha256")
            s_sha = s_meta.get("sha256")
            if t_sha == s_sha:
                msg = (
                    f"[RAIDEN PARITY VERIFIED | Step {step_key}] Rank 0: 100% BIT-FOR-BIT WEIGHT MATCH!\n"
                ...
                logger.error(
                    f"[RAIDEN PARITY MISMATCH | Step {step_key}] Rank 0 weights do NOT match trainer!\n"
                ...
                t_pt = trainer_checksum.get("per_tensor", {})
                s_pt = s_meta.get("per_tensor", {})
                if t_pt and s_pt:
                    mismatches = []
                    for name, t_info in t_pt.items():
                        s_info = s_pt.get(name)
                        if s_info is None:
                            mismatches.append(f"  * Missing on Sampler: {name}")
                        elif t_info.get("sha256") != s_info.get("sha256"):
                            l1_diff = abs(t_info.get("l1", 0) - s_info.get("l1", 0))
                            l2_diff = abs(t_info.get("l2", 0) - s_info.get("l2", 0))
                            mismatches.append(
                                f"  * {name}: Trainer(L1={t_info.get('l1'):.6f}, L2={t_info.get('l2'):.6f}) vs "
                                f"Sampler(L1={s_info.get('l1'):.6f}, L2={s_info.get('l2'):.6f}) [delta_L1={l1_diff:.6f}, delta_L2={l2_diff:.6f}]"
                            )
                    logger.error(
                        f"[RAIDEN PARITY MISMATCH | Step {step_key}] {len(mismatches)} / {len(t_pt)} tensors differed:\n"
                        + "\n".join(mismatches[:30])
                    )
    except Exception as e:
