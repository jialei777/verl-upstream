...
class RaidenCheckpointEngine(CheckpointEngine):
    ...
    async def send_weights(
        ...
        **kwargs,
        ...
        if hasattr(self, "_trainer_raiden_ws") and self._trainer_raiden_ws is not None:
            try:
                del self._trainer_raiden_ws
            except Exception:
                pass
            self._trainer_raiden_ws = None
        ...
        try:
            ...
            print(
                f"@@@ [RAIDEN DIAG | Trainer] Step {step_key}: host_buf(0,0) L1={hb_l1:.6f}, "
                f"metrics: d2h_count={m.get('d2h_call_count')}, last_d2h_bytes={m.get('last_d2h_bytes')}, "
                f"total_d2h_bytes={m.get('total_d2h_bytes')}, total_h2h_bytes={m.get('total_h2h_bytes')}",
                flush=True,
            )
